...
help:
	...
	@echo "  docker run       Run application in Docker (HTTP mode, 6 core tools)"
	@echo "  docker run-http  Run in Docker HTTP mode (port 8084, 6 core tools)"
	@echo "  docker run-stdio Run in Docker stdio mode (for MCP clients, 6 core tools)"
	@echo "  docker run-dev   Run in Docker development mode (42 tools)"
	@echo "  docker stop      Stop Docker containers"
	@echo "  docker clean     Clean Docker resources"
	@echo ""
	@echo "  NOTE: Use docker run-dev for all 42 tools, or modify docker-compose.yml"
	@echo "        to add -all-tools flag to other services"
	@echo ""
